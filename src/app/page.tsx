...
const FEATURE_CARDS = [
  {
    lines: ['흩어진 간식 구매처를 통합하고,', '기수별 지출을 똑똑하게 관리하세요.'],
  },
  {
...
function FeatureCard({ lines }: { lines: readonly string[] }) {
  return (
    <div className="mr-3 flex shrink-0 items-center justify-center rounded-lg border border-solid border-[#e4e4e4] bg-white/40 px-6 py-5 shadow-[0px_7px_20px_0px_rgba(0,0,0,0.02)] backdrop-blur-[20px] sm:mr-5 sm:px-[30px] sm:py-6 lg:mr-10 lg:p-[30px]">
...
function FeatureMarquee() {
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="flex w-max animate-landing-marquee will-change-transform">
        {cards.map((card, index) => (
          <FeatureCard
            key={`${index}-${card.lines[0]}`}
            lines={card.lines}
          />
        ))}
      </div>
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-white">
      ...
      <main className="flex flex-col items-center pb-0 pt-[120px] sm:pt-[170px] lg:pt-[171px]">
        <h1 className="px-4 text-center text-[20px] font-extrabold leading-normal tracking-[-0.5px] text-gray-950 sm:px-0 sm:text-[32px] sm:tracking-[-0.8px] lg:text-[50px] lg:leading-[1.4] lg:tracking-[-1.25px]">
        ...
        <section className="relative mt-10 w-full sm:mt-16">
